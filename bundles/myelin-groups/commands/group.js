...
module.exports = srcPath => {
  ...
  subcommands.add({
    ...
    command: state => (args, player) => {
      ...
      const potentialPartiers = [...player.room.players, ...player.room.npcs]
        .filter(char => !char.isNpc || char.hasBehavior('group'));

      const target = Parser.parseDot(args, potentialPartiers);
  ...
  subcommands.add({
    ...
    command: state => (args, player) => {
      if (!player.party) {
        say(player, "You're not in a group.");
        say(player, "Try <b>group create</b> or <b>group join</b>");
      }
